...
def generic_fields(name, seen=None):
    ...
    for pname, pdef in (sdef.get("properties") or {}).items():
        ...
        if isinstance(pdef.get("items"), dict):
            refs.append(pdef["items"].get("$ref", ""))
        if any("GenericObjectRequest" in r for r in refs):
            out.add(pname)
    for sub in sdef.get("allOf", []):
        if isinstance(sub, dict) and "$ref" in sub:
            out |= generic_fields(sub["$ref"].split("/")[-1], seen)
    return out
...
for path_name, ops in data.get("paths", {}).items():
    ...
    for method in ("post", "put", "patch"):
        ...
        for sname in request_schema_names(op):
            ...
            entries.setdefault(f"{app}.{model}", set()).update(fields)

pairs = sorted((m, f) for m, fs in entries.items() for f in fs)
rows = "".join(f'    ("{m}", "{f}"),\n' for m, f in pairs)

content = f'''//! Generic foreign-key (`GenericObjectRequest`) fields, derived from the
...
pub fn is_generic_fk(model: &str, field: &str) -> bool {{
    GENERIC_FK_FIELDS.iter().any(|(m, f)| *m == model && *f == field)
}}
'''
