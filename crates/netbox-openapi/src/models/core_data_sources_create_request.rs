/*
 * NetBox REST API
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 4.7.0-Docker-5.1.1 (4.7)
 *
 * Generated by: https://openapi-generator.tech
 */

#[derive(Clone, Debug, PartialEq, Default, Serialize, Deserialize)]
pub struct CoreDataSourcesCreateRequest {
    #[serde(rename = "name")]
    pub name: String,
    #[serde(rename = "type")]
    pub r#type: String,
    #[serde(rename = "source_url")]
    pub source_url: String,
    #[serde(rename = "enabled", skip_serializing_if = "Option::is_none")]
    pub enabled: Option<bool>,
    #[serde(rename = "description", skip_serializing_if = "Option::is_none")]
    pub description: Option<String>,
    /// * `1` - Minutely * `60` - Hourly * `720` - 12 hours * `1440` - Daily * `10080` - Weekly * `43200` - 30 days
    #[serde(
        rename = "sync_interval",
        default,
        with = "::serde_with::rust::double_option",
        skip_serializing_if = "Option::is_none"
    )]
    pub sync_interval: Option<Option<SyncInterval>>,
    #[serde(
        rename = "parameters",
        default,
        with = "::serde_with::rust::double_option",
        skip_serializing_if = "Option::is_none"
    )]
    pub parameters: Option<Option<serde_json::Value>>,
    /// Patterns (one per line) matching files or paths to ignore when syncing
    #[serde(rename = "ignore_rules", skip_serializing_if = "Option::is_none")]
    pub ignore_rules: Option<String>,
    #[serde(
        rename = "owner",
        default,
        with = "::serde_with::rust::double_option",
        skip_serializing_if = "Option::is_none"
    )]
    pub owner: Option<Option<Box<crate::models::AsnRangeRequestOwner>>>,
    #[serde(rename = "comments", skip_serializing_if = "Option::is_none")]
    pub comments: Option<String>,
    #[serde(rename = "custom_fields", skip_serializing_if = "Option::is_none")]
    pub custom_fields: Option<::std::collections::HashMap<String, serde_json::Value>>,
}

impl CoreDataSourcesCreateRequest {
    pub fn new(name: String, r#type: String, source_url: String) -> CoreDataSourcesCreateRequest {
        CoreDataSourcesCreateRequest {
            name,
            r#type,
            source_url,
            enabled: None,
            description: None,
            sync_interval: None,
            parameters: None,
            ignore_rules: None,
            owner: None,
            comments: None,
            custom_fields: None,
        }
    }
}

/// * `1` - Minutely * `60` - Hourly * `720` - 12 hours * `1440` - Daily * `10080` - Weekly * `43200` - 30 days
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum SyncInterval {
    #[serde(rename = "1")]
    Variant1,
    #[serde(rename = "60")]
    Variant60,
    #[serde(rename = "720")]
    Variant720,
    #[serde(rename = "1440")]
    Variant1440,
    #[serde(rename = "10080")]
    Variant10080,
    #[serde(rename = "43200")]
    Variant43200,
    #[serde(rename = "null")]
    Null,
}

impl Default for SyncInterval {
    fn default() -> SyncInterval {
        Self::Variant1
    }
}
